...
//edited by Donny Ali Sanjaya & Gemini 3.5 Flash
...
{
  Serial.printf("--> Start: New Connection, from nodeId = %u\n", nodeId); //informasi nodeID dari node baru
...
void setup() 
{
  ...
  // Memonitor Free Heap RAM saat booting
  Serial.printf("Initial Free Heap RAM: %d bytes\n", ESP.getFreeHeap());
  ...
  {
    // Log RAM heap sebelum memproses request
    uint32_t freeHeapBefore = ESP.getFreeHeap();
    String response = mesh.subConnectionJson(false);
    request->send(200, "application/json", response);
    Serial.printf("[HTTP /scan] Free Heap Before: %d bytes, After: %d bytes\n", freeHeapBefore, ESP.getFreeHeap());
  });
